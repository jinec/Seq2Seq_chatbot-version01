...
def _extract_argmax_and_embed(embedding,
  ...
  def loop_function(prev, _):
    if output_projection is not None: #先投影，之后再argmax
      prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])
    prev_symbol = math_ops.argmax(prev, 1) #找最大的那一类的id号
    # Note that gradients will not propagate through the second parameter of embedding_lookup.
    emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)#查找最大一类的embedding内的【i】行
    ##embedding（6865,512）中有可能出现2行一样的吗？没有！出现的话，不符合embedding的映射条件了：有唯一的x与之相对应
...
def embedding_rnn_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_decoder") as scope:
    ...
    loop_function = _extract_argmax_and_embed(
        ...
        update_embedding_for_previous) if feed_previous else None #我对这个张量的维度，表示很好奇。
...
def attention_decoder(decoder_inputs,
  ...
  Args:
    ...
    num_heads: Number of attention heads that read from attention_states.#类似于用了几个卷积核
    loop_function: If not None, this function will be applied to i-th output
      in order to generate i+1-th input, and decoder_inputs will be ignored,
      except for the first element ("GO" symbol). This can be used for decoding,
      but also for training to emulate http://arxiv.org/abs/1506.03099.
    ...
    initial_state_attention: If False (default), initial attentions are zero.此时，状态来自输入！
      If True, initialize the attentions from the initial state and attention
      states -- useful when we wish to resume decoding from a previously
      stored decoder state and attention states.#这里有难度？
  ...
  with variable_scope.variable_scope(
    ...
    #(?,5,1,512)   1：占位，为了卷积加的维度[batch_size,H,W,Channel]；2d的效果更好
    hidden_features = []
    v = []
    #attention_vec_size 512
    attention_vec_size = attn_size  # Size of query vectors for attention.   k为PPT中的U
    for a in xrange(num_heads): #如果num_heads》1，就有多个卷积核了
      k = variable_scope.get_variable("AttnW_%d" % a,
                                      [1, 1, attn_size, attention_vec_size]) #
      hidden_features.append(nn_ops.conv2d(hidden, k, [1, 1, 1, 1], "SAME"))#conv2d的输入必须是4维；1d是x，取代了（H，w）；
    ...
    def attention(query):     #query是(递归状态:(state))，两层模型
      ...
      if nest.is_sequence(query):  # If the query is a tuple, flatten it.
        ...
        for q in query_list:  # Check that ndims == 2 if specified.
          ...
          if ndims:
            assert ndims == 2
        query = array_ops.concat(query_list, 1)#shape=2048，why？new_c+new_h=1024,因为有2层，所以是2048
      for a in xrange(num_heads):#a 是 位置序号
        with variable_scope.variable_scope("Attention_%d" % a):
          y = linear(query, attention_vec_size, True)    #这里有参数Wa         #pre_hiddent_state（si-1）*W
          y = array_ops.reshape(y, [-1, 1, 1, attention_vec_size])
          # Attention mask is a softmax of v^T * tanh(...).
          s = math_ops.reduce_sum(v[a] * math_ops.tanh(hidden_features[a] + y),
                                  [2, 3])#hidden_features=图画中的Ua*hj，Ua是那个卷积过程
          a = nn_ops.softmax(s)
    ...
    outputs = []
    prev = None
    #batch_size = 64 ;attn_size = 512
    batch_attn_size = array_ops.stack([batch_size, attn_size])#句数，句长.；维度叠加
    #初始化一个[64,512]的矩阵
    attns = [
        array_ops.zeros(
            batch_attn_size, dtype=dtype) for _ in xrange(num_heads)
    ]#用来打padding
    for a in attns:  # 在每个句子中，Ensure the second shape of attention vectors is set.
      a.set_shape([None, attn_size])
    if initial_state_attention: #此时，是false，这样只能用上一步的0值了
      attns = attention(initial_state) #这一步跟下面不冲突吗？重复赋值了啊！答：不冲突，后面是为下一步做准备！attns是初始化的结果c=0；
    for i, inp in enumerate(decoder_inputs):
      if i > 0:
        variable_scope.get_variable_scope().reuse_variables()#图重用刚才t=0时的变量
      # If loop_function is set, we use it instead of decoder_inputs.
      if loop_function is not None and prev is not None: #T=0时，没有执行
        with variable_scope.variable_scope("loop_function", reuse=True):
          inp = loop_function(prev, i)#将上一时刻的输出分类，embeding后作为分类
      # Merge input and previous attentions into one vector of the right size.
      input_size = inp.get_shape().with_rank(2)[1] #512
      if input_size.value is None:
        raise ValueError("Could not infer input size from input: %s" % inp.name)
      #inp是PPT中的解码输入， attns是PPT中的C;
      x = linear([inp] + attns, input_size, True)#t=0时，[inp]是输入，go/start；拼接
      # Run the RNN.    输入state是上一个状态pre_hidden_state,输出state是hidden_state
      cell_output, state = cell(x, state)#cell前，3根线回合完毕；
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + attns, output_size, True)#y（i-1）和c（i-1）拼接起来，做了一次类似全卷积
      if loop_function is not None:  #注意，在上一步中 attns被计算了2次
        prev = output  #将上一时刻的输出作为输入
      outputs.append(output)
...
def embedding_attention_decoder(decoder_inputs,
  ...
  """
  if output_size is None:
    output_size = cell.output_size#也是512
  #output_projection为(w,b)
  if output_projection is not None:
    proj_biases = ops.convert_to_tensor(output_projection[1], dtype=dtype)
    proj_biases.get_shape().assert_is_compatible_with([num_symbols])
  ...
  with variable_scope.variable_scope(
    ...
    embedding = variable_scope.get_variable("embedding",
                                            [num_symbols, embedding_size])#embedding.shape=（6865，12）
    loop_function = _extract_argmax_and_embed(
        embedding, output_projection,
        update_embedding_for_previous) if feed_previous else None   #功能：分类，取最大值 + embedding，嵌入，作输入
    emb_inp = [
        embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs
    ]#由embedding的原理知，不可能有2行一模一样的；直接查emb_inp，代替了 one-hot过程；问题来了，这一步的结果emb_inp用在哪里？
    #https://blog.csdn.net/u013041398/article/details/60955847
    return attention_decoder(
        emb_inp,                  #解码器输入
        initial_state,            #编码器最后一个状态作为解码器初始向量
        attention_states,         #编码器输出
        cell,
        output_size=output_size,  #512
        num_heads=num_heads,
        loop_function=loop_function, #前一个预测值作为的输入
...
def embedding_attention_seq2seq(encoder_inputs,
                                ...
                                num_decoder_symbols,
                                embedding_size,
                                num_heads=1,  #好几个版本始终未1，具体老师也不知道
                                output_projection=None,
                                feed_previous=False,
                                dtype=None,
                                scope=None,
  ...
  with variable_scope.variable_scope(
    ...
    dtype = scope.dtype
    # Encoder.
    encoder_cell = copy.deepcopy(cell) #这个cell记得吗？是前面两层被封装在一起后的cell
    encoder_cell = core_rnn_cell.EmbeddingWrapper(
        encoder_cell,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)#6865到512的映射
    #进行类似特征提取，得出编码器的状态和输出
    encoder_outputs, encoder_state = rnn.static_rnn(   #此处被LSTM的运作方式进行了设置
        encoder_cell, encoder_inputs, dtype=dtype)#取encoder_state的-1就是它最后的状态

    # First calculate a concatenation of encoder outputs to put attention on.
    top_states = [
        array_ops.reshape(e, [-1, 1, cell.output_size]) for e in encoder_outputs
    ]#output_size=512.加了1维，具体为[batch_size（句数），字的个数(句长)，字的向量维度]
    attention_states = array_ops.concat(top_states, 1) #聚合，shape=（？，5，12）
    #batch_size*H*W*channel  
    # Decoder.
    output_size = None
    if output_projection is None:
      cell = core_rnn_cell.OutputProjectionWrapper(cell, num_decoder_symbols)
      output_size = num_decoder_symbols#无论如何，把输出的向量维度升上去

    if isinstance(feed_previous, bool):
      return embedding_attention_decoder(
          decoder_inputs,
          encoder_state,
          attention_states,       #编码器的输出序列
          cell,                   #解码器2层cell
          num_decoder_symbols,    #6865
          embedding_size,         #512
          num_heads=num_heads,    #1
          output_size=output_size,#即解码器的输出维度，近似于512
          output_projection=output_projection,#输出映射。transpose：0，shape=【512，865】；proj_b：0，shape=（6865，）
          feed_previous=feed_previous,#f True, only the first of decoder_inputs will be used (the "GO" symbol)。
          initial_state_attention=initial_state_attention)#false，说明：不随机
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state = embedding_attention_decoder(
            ...
            output_projection=output_projection, #shape=(512,6865)
            feed_previous=feed_previous_bool,
            update_embedding_for_previous=False,
            initial_state_attention=initial_state_attention)#状态来自 编码器，不能false
        state_list = [state]
        if nest.is_sequence(state):
          state_list = nest.flatten(state)
        return outputs + state_list
...
def sequence_loss_by_example(logits,
                             ...
                             name=None):#一个关于 每位元素的softmax_loss_function的列表
  ...
  with ops.name_scope(name, "sequence_loss_by_example",
    ...
    for logit, target, weight in zip(logits, targets, weights):
      ...
      log_perp_list.append(crossent * weight)#weight 是0/1值的吗？
    log_perps = math_ops.add_n(log_perp_list)
    if average_across_timesteps: #关于 weight的一个归一化
      total_size = math_ops.add_n(weights)#列表元素相加
...
def model_with_buckets(encoder_inputs,
                       ...
                       weights,
                       buckets,
                       seq2seq,  #这是把seq2seq_f（）当做一个参数传进来了
                       softmax_loss_function=None,#这是把 softmax_loss_function（）当做一个参数穿进来了
  ...
  all_inputs = encoder_inputs + decoder_inputs + targets + weights #链表之间的相加，相当于拼接。#target没有第1位的go，长度为30
  losses = []
  outputs = []
  with ops.name_scope(name, "model_with_buckets", all_inputs): #这个署名只是为了在 tensorboard上看，是否有这个东西
    for j, bucket in enumerate(buckets): #在每一个桶里
      with variable_scope.variable_scope(
          variable_scope.get_variable_scope(), reuse=True if j > 0 else None):#参数从第2个桶开始共享？
        bucket_outputs, _ = seq2seq(encoder_inputs[:bucket[0]], #参数x
                                    decoder_inputs[:bucket[1]]) #参数y
        outputs.append(bucket_outputs)
        if per_example_loss:
          losses.append(
              sequence_loss_by_example(
                  outputs[-1], #即外面的x值
                  targets[:bucket[1]],#即外面的y值.
                  weights[:bucket[1]],#从前面的长度31变为应该的 25/30/10
                  softmax_loss_function=softmax_loss_function))
        else:
          losses.append(
              sequence_loss(
                  outputs[-1],#取得是最后一批回答的预测来进行对比
                  targets[:bucket[1]],
                  weights[:bucket[1]],
                  softmax_loss_function=softmax_loss_function))
